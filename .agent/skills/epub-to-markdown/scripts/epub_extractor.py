...
def extract_chapters(epub_path, output_dir):
    with zipfile.ZipFile(epub_path, 'r') as z:
        ...
        with z.open(nav_path) as f:
            nav_content = f.read().decode('utf-8')

        # Find all <li><a> mapping
        # Example: <li><a href="xhtml/Section0001_0002.xhtml">第一章 立志</a></li>
        pattern = re.compile(r'<li>\s*<a href="([^"]+)">([^<]+)</a>\s*</li>', re.DOTALL)
        matches = pattern.findall(nav_content)
        ...
        for href, title in matches:
            ...
            # File inside zip is relative to EPUB/
            internal_path = "EPUB/" + href
            if internal_path in z.namelist():
                print(f"Extracting {title}...")
                with z.open(internal_path) as f_ch:
                    html_content = f_ch.read().decode('utf-8')
